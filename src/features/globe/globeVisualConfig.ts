export const EARTH_RADIUS = 1;
export const ATMOSPHERE_RADIUS = 1.2;
export const CAMERA_DISTANCE = 2.8;
export const CAMERA_FOV = 45;

export const EARTH_TEXTURE_DAY = '/textures/earth/earth-day.jpg';
export const EARTH_TEXTURE_NIGHT = '/textures/earth/earth-night.jpg';
export const STARFIELD_TEXTURE = '/textures/earth/starfield.jpg';

export const PIN_DOT_RADIUS = 0.012;
export const PIN_DOT_COLOR = 0x4fcf8f;
export const PIN_GLOW_COLOR = 0x4fcf8f;
export const PIN_GLOW_OPACITY = 0.3;

export const AUTO_ROTATION_SPEED = 0.04;
export const ZOOM_MIN = 1.5;
export const ZOOM_MAX = 5;

export const GLOBE_CONTAINER_HEIGHT_DESKTOP = '100%';
export const GLOBE_CONTAINER_HEIGHT_MOBILE = '60vh';
